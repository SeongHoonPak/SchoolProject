...
export async function getByChatId(id, userId) {
  db.execute(`${select} FROM chats as ch JOIN users as us ON ch.productId=? AND ch.userId=? AND us.id = ch.userId`, [id,userId]).then(result=> 
